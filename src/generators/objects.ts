...
const objectGenerators: ObjectInputGenerator[] = [
    ...
    },
    {
        ...
        generate: (): ValueWithDescription[] => [
            ...
            { value: (() => { const createTree = (depth: number, breadth: number): any => depth <= 0 ? { leaf: true } : Object.fromEntries(Array.from({ length: breadth }, (_, i) => [`child${i}`, createTree(depth - 1, breadth)])); return createTree(8, 8); })(), description: "(() => { const createTree = (depth: number, breadth: number): any => depth <= 0 ? { leaf: true } : Object.fromEntries(Array.from({ length: breadth }, (_, i) => [`child${i}`, createTree(depth - 1, breadth)])); return createTree(8, 8); })()" }
        ]
    },
    {
        category: 'objects',
